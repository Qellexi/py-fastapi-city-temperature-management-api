...
async def create_city(city: schemas.CityCreate, db: AsyncSession = Depends(get_db)):
    db_city = await crud.create_city(db, city)
    if db_city:
        raise HTTPException(
            status_code=400,
...
async def get_city_by_id(city_id: int, db: Annotated[AsyncSession, Depends]):
    return await crud.get_city_by_id(db=db, city_id=city_id)

@router.put("/cities/{city_id}", response_model=schemas.City)
async def update_city_by_id(city_id: int, city_data: schemas.CityUpdate, db: Annotated[AsyncSession, Depends]):
    try:
...
async def delete_city_by_id(city_id: int, db: Annotated[AsyncSession, Depends]):
    try:
